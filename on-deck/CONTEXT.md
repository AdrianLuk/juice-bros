...
**Club QR**:
A single stable QR code, and the join link behind it, that Players use to join - resolving always to that Club's currently-open Session, or to a "nothing running right now" screen, and never reprinted or regenerated per Session. The pinned link is a first-class way in, not a fallback for the code: it is the first item of the first-night kit (issue #521) precisely because most of a first Saturday's room can be admitted from the group chat before anyone has arrived, with nothing printed and nothing shown. Three ways an Organizer puts it in front of Players, none more canonical than the others: pasted into the club's group chat and pinned there, with a pre-written message a copy control writes alongside the link; printed once on a sign for the venue (`/on-deck/home/qr`, Organizer-gated, prints on Letter or A4); or held up full screen from the Organizer's own phone (`/on-deck/home/qr/hold-up`, issue #517) for a night without one, which carries the same copy-the-message control for a link posted late. The bare code is also served as a file at `/on-deck/c/<clubId>/qr.svg` and `qr.png`, which are open, like the link they encode.
...
**First-night kit**:
Two rows on home (issue #521, parent #512), shown only for a Club that has never closed a Session: put the join link in the group chat, and have the Club QR ready for walk-ups. Not a general onboarding checklist - the Kiosk and the Volunteer Link are capabilities an Organizer discovers from the floor screen once a Session is running (see Floor Mode), never steps presented here, because for this release's solo organizer both usually resolve to "skip this" and a checklist half of whose steps are optional teaches that the product is more complicated than it is. Gone the moment the Club's first Session closes; the same join message resurfaces on that Session's own Summary page, because the weekly rhythm still needs it and the kit itself does not come back.
_Avoid_: Onboarding, Setup checklist (both imply an ongoing series of steps to work through; this is exactly two, shown once).
...
Below the board is the way out: a "Create your Club" that goes to sign-in, so the moment somebody is convinced and the moment they can act are the same moment rather than two different pages (issue #524). It is the one click in the funnel that fires before there is an account to attribute it to.
...
**Funnel Event**:
One of six Vercel Analytics events tracing a stranger from the Demo night to their first closed Session (issue #524): `od_demo_opened`, `od_demo_finished`, `od_club_intent`, `od_club_created`, `od_first_session_started`, `od_first_session_closed`. Named in `src/lib/on-deck/analytics-events.ts`. The last three are emitted server-side from `src/lib/on-deck/analytics.ts` inside `after()`, each gated on a database check the client cannot be trusted to make and each firing once on the 0→1 transition; the first three fire from the browser, anonymously, because the demo has no account behind it and `od_club_intent` deliberately precedes sign-in. No PII, no table of our own, no dashboard - read by hand in the Vercel Analytics console, see [docs/adoption-funnel.md](docs/adoption-funnel.md) and [adr/0008-adoption-funnel-analytics.md](docs/adr/0008-adoption-funnel-analytics.md).
Events are keyed to a visitor session and carry nothing to join on, so a visitor who demos today and creates a Club next week is two unrelated sessions: the three demo-side events and the three account-side ones cannot be joined across that gap, and a conversion rate spanning it is an estimate.
_Avoid_: Metric, KPI, Conversion (as the name of the event itself).

## Playing

**Player**:
Someone playing in a Session, identified by a first name plus last initial and a self-declared Skill Level. Created by scanning the Club QR or tapping its pinned join link and doing a two-tap setup, or by a Volunteer adding a walk-up. Scoped entirely to one Session: On Deck holds no accounts, no phone numbers, and no memory of a Player between Sessions. There is no check-in and no door process - a Player exists the moment they enter themselves or a Volunteer enters them, which is not necessarily when they arrived.
