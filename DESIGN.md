...
**Every step keeps its floor above the next step's.** A clamp whose lower bound collapses onto its neighbour's deletes the whole channel at the viewport where most visitors are: the first version of this ladder bottomed out at 24/22/20px below 640px, a 4px spread across four ranks, so on a phone the page had no heading hierarchy at all. The floors are now 36/28/22/18/16px from the h1 down, and a new step is checked at 390px before it is checked at 1440 — the h1 itself sat at 32px against the peak step's 28px until the same check caught it, which had made the page's largest step its least obvious one.
