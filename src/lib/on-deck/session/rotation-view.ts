/**
 * The rotation loop's read model (issue #243) — the projection from a folded
 * Session to what a board renders. Every live surface — the Organizer floor
 * screen and a Player's own "where am I" line — re-fetches this (via the
 * `getRotationView` Server Action, in `../actions/rotation.ts`) on a Realtime
 * notify (issue #252), falling back to a ~4s TanStack Query poll when the
 * socket drops, and re-renders.
 *
 ...
 * projection in this module takes. Defined here (not in `../sessions.ts`,
 * which loads it) because every field is already pure: `../sessions.ts`
 * re-exports this type rather than duplicating it.
 */
