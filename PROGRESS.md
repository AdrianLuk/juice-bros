...
- [x] **3.2 Sitemap and robots**
  ...
  - [x] `app/robots.ts` exists, references the sitemap; allows the marketing/podcast site and Booking Buddy's public entry points (`/booking-buddy`, sign-in, privacy), disallows Booking Buddy's signed-in-only feature routes (friends/groups/orgs/bookings/slots/settings, built from `routes.ts` constants) and the tokenised guest Slot Links (`/s/`)
