...
- [x] **3.5 Performance check** — Lighthouse (v12, simulated) run against `next start` for the homepage and an episode page, mobile + desktop. Desktop is 100 on both. Episode pages: mobile 98, LCP 2.4s. Homepage mobile started at 89 with a 4.2 MB page and a 3.6s LCP; the fixes below took it to 91 / 903 KB. Every unthrottled *observed* metric is already well inside budget (observed LCP 134ms, CLS 0, last visual change 169ms) — the residual mobile-lab LCP is Lantern's slow-CPU render-delay estimate for a full-bleed hero image plus framework hydration, not a real load problem. Vercel Speed Insights is wired, so real field data will land once traffic ramps.
  - **YouTube embed facade** (`src/components/youtube-embed.tsx`): the featured-episode embed (homepage) and the About origin-story embed are now click-to-load — poster thumbnail + play button until the viewer hits play, then the real autoplay iframe. Drops ~830 KB of youtube.com player JS from first paint on both pages; homepage TBT 60ms → 25ms, total bytes 4.2 MB → 0.9 MB.
  - **Hero LCP image**: now hand-built responsive WebP (`public/brand/JB_Banner-768/1280/1600.webp`, 57/121/163 KB) served via `<picture>`, with the original `JB_Banner_1920.jpeg` kept as the `<img>` fallback, plus `fetchPriority="high"` + `decoding="async"`. Mobile fetches 57 KB instead of 273 KB. _(Superseded by #414: the variants are now generated by `npm run optimize:images` from `src/lib/image-variants.ts`, AVIF sits ahead of WebP, and the duplicate `JB_Banner_1920.jpeg` master was dropped in favour of `JB_Banner.jpeg`.)_
  - **Featured-episode background wash**: was a 2.3 MB PNG (`adrian-dav-chatgpt-edited.png`); swapped for a 55 KB WebP, PNG deleted.
  - `getYoutubeEmbedUrl` (left unused after the facade) replaced by `getYoutubeVideoId` in `src/lib/utils.ts`, covered by new `src/lib/utils.test.ts`.
  - Verified: `npm test` (531/531), `tsc --noEmit`, `eslint`, `next build`, plus a Playwright check confirming the facade swaps to a live autoplay iframe on click.
