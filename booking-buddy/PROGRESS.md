...
- **`isReminderDue`'s window is deliberately wide, not a narrow "this exact minute" check.** It's due from the moment `now` reaches the configured offset before `proposedStart`, and stays due all the way up to the Slot's own start. That's what makes the job safe to run infrequently: **Vercel's Hobby plan only permits daily cron invocations**, and a narrow window checked once a day would miss most Reminders entirely. `vercel.json` still asks for `*/15 * * * *` — correct on any paid plan, and worth keeping so nothing has to change if this project upgrades — but on Hobby it silently collapses to once a day. `reminder_sends` is what actually prevents a duplicate send, not the window's width, so widening it costs nothing.
