...
Two follow-ups are open. **Neither is next, even though #18 is now buildable:**

- [#18](https://github.com/AdrianLuk/juice-bros/issues/18) — the Google Places half. The API key landed (2026-08-14), so this is relabeled `ready-for-agent`. **Deliberately not picked up before #8**: it upgrades Org creation, which already works end-to-end via the hand-typed path shipped in #7, and it doesn't unblock anything else in the roadmap. #8 does — #9, #10 and #11 all sit downstream of Slots existing. When there's a session to spare that isn't the critical path, this is it.
- [#20](https://github.com/AdrianLuk/juice-bros/issues/20) — `time_zone` belongs on `orgs`, not `bookings`. Blocked on #18 for the coordinates that make deriving it automatic. Comes after #18, whenever that happens.

**Start the next session with**: `gh issue view 8` (Phase 5, Slot as a poll). It's the linchpin of everything left in the plan — #9 (confirming a Slot), #10 (Slot Link/Guest) and #11 (Reminders) all build directly on Slots existing, and it's also where the app starts doing the thing it's for: CONTEXT.md's whole premise is "post open time, see who's in," and nothing built so far does that yet. Note `gh issue view` fails on this repo with a Projects-classic GraphQL error — use `gh issue view 8 --json number,title,body,comments` or `gh api`. Same for `gh pr edit`; `gh issue edit`, `gh issue create` and `gh api` all work.

**Model note for #8**: Sonnet is enough. Almost all of it is extending an already-established recipe — DAL, `ActionResult`, pure-function extraction, the pgTAP/Playwright split, and `resolveVisibility` itself are all built and just get called. The one genuinely new piece is the optimistic-update mutation (9.3's dedicated UI test) — the first real TanStack Query mutation in Booking Buddy, versus the search box's query-only usage. If that slice feels shaky mid-build, that's the moment to reach for Opus on just that piece, not a reason to start the ticket there.
