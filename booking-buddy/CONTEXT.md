...
**Import Candidate**:
A parsed CourtReserve confirmation or cancellation email, matched against the User's Orgs and existing Bookings but not yet applied — shown on a review screen for the User to confirm before it becomes (or removes) a real Booking. Never a Booking itself; discarding one has no effect on any Booking. Skipped automatically, without ever becoming a candidate the User sees, when it's a confirmation for a date/time that's already passed or that duplicates an existing Booking (same Org, court, and date/time).
