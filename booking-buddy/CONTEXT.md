...
Bookings under an Org are still entered by hand: Google supplies the venue's identity, not its reservations (ADR 0002 stands). An Org may also carry a Booking Window, driving the Booking Reminder. One Org per owner may be marked the **default facility**, pre-selecting it in the Booking form's picker instead of forcing an explicit pick every time; picking a different Org as default simply moves the mark, there is no "unset."
