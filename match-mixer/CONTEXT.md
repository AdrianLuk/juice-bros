...
Its source is kept dormant at `src/components/apps/match-mixer/partner-matrix.tsx`, with the CSS it needs beside it in `partner-matrix.retired.css`. Nothing imports either, and the build ships neither. They are in the tree rather than left to `git` because PR #478 was squash-merged and both branches deleted, which leaves the commits holding them prunable. Keeping them is a hedge against losing the work, not a plan to render it: reviving the surface as-is reopens everything above, so if it comes back it should come back answering "which Rounds", which is the thing it could never do.
