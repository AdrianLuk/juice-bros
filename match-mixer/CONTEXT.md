...
Since RR-6.2 (#553) order carries a second deliberate meaning, on top of fixed partners' first. A line beginning `---` is a **Pool header**, and where a line sits relative to one is which **Pool** it is in — everything below a header until the next is that Pool, and a leading header is optional, so a list with one `---` in the middle is Pool A and Pool B without the organizer typing a header for the group already at the top. [ADR 0005](docs/adr/0005-the-roster-declares-the-pools.md) argues why a Roster convention rather than a control on the Config; the **Pool** entry describes what a header does once it lands.

A line may also end in a **Marker**, a single trailing `M` or `F` that **mixed doubles** reads. It rides on the roster line and never in a field of its own, which is what keeps the parse honest — there is no range to guess at and no question of whether a name ends in one — and what lets it travel: the Share Link carries lines rather than names, so a marked board opens marked. A line that is only a Marker is a name, because there is nothing before it to attach one to. A Pool header is read before a Marker and before anything else on the line, so `--- F` under mixed doubles is a Pool called "F" and never a lone `F`.
...
**Pool** (RR-6, #392; the count shipped in RR-6.1, #552; the Roster's own headers in RR-6.2, #553):
...
**Pool headers** are the other way to say who is in which Pool ([ADR 0005](docs/adr/0005-the-roster-declares-the-pools.md)). A line in the Roster beginning `---` starts a Pool; it may carry a label (`--- 4.0`), which becomes the Pool's name wherever the board would otherwise say "Pool A" — on the board, in the summary lines, in refusal messages and on paper. A bare `---` falls back to its letter by position, and a leading header is optional, so a list with one `---` in the middle is Pool A and Pool B without a header typed for the group already at the top.

**Headers win.** When the Roster has headers, they are the Pools and the Pool count control goes quiet, saying the split is set by the Roster; remove every header and the count takes the split back. A declared split is sized and composed exactly as typed rather than dealt — nothing here rebalances it — so every per-Pool rule holds against what the organizer actually put in each Pool: 4 to 32 Players, the mixed-doubles arithmetic read off that Pool's own Markers rather than an even deal, and in fixed partners each Pool paired from its own top, an odd Pool refused by name the same way an odd Roster is.

**Keep this split** bridges the two mechanisms: it writes the board's current Pools into the roster box as bare `---` lines, after which the split belongs to the organizer and a name can be moved across a line by hand. It is the only thing in the tool that ever writes to the roster box, and only when pressed — it is also how an organizer discovers the header syntax in the first place, findable off a board that is already split rather than buried in a note.
